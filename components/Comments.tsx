    ...
    return (
        <div className='border-t-2 border-gray-200 pt-4 px-10 mt-4 bg-[#F8F8F8] border-b-2 lg:pb-0 pb-[100px]'>
            <div className='overflow-scroll lg:h-[457px]'>
                {comments?.length > 0 ? comments?.map((item: IComment, idx: number) => (
                    <div key={idx}>
                        {allUsers?.map((user: IUser, index: number) => user?._id === (item?.postedBy?._ref || item?.postedBy?._id) && (
                    ...
                    <NoResults
                        text='No Comments Yet! Be First to add a comment.'
                    />
                )}
            </div>
            {userProfile && (
                <div className='absolute bottom-0 left-0 pb-6 px-2 md:px-10'>
                    <form className='flex gap-4' onSubmit={addComment}>
                        ...
                        />
                        <button
                            className='xl:border-2 hover:bg-primary xl:border-[#F51997] px-3 py-2 rounded xl:rounded-full flex items-center gap-2 justify-center cursor-pointer text-[#F51997] font-semibold'
                            onClick={addComment}
                        >
